...
The simplest and one of the most robust classifiers is k-NN [5]. It doesn't generalize information, instead it saves training dataset and during prediction it finds the most similar historical observations and predicts label of a new observation based on their labels. However, it not only doesn't have the capacity to distinguish important features from not important ones, but also to find more complex interactions between variables. 

One way to improve k-NN's performance is to precede closest neighbor computation with transformation of the space of observations, so that the derivative variables are more meaningful. Such beneficial transformation is called embedding. It can be done in various ways. 
...
An autoencoder consists of two parts, the encoder and the decoder, which can be defined as functions \phi and \psi such that:
...
Logistic regression is basic classification model [1]. We get probability of belonging to given class by:
...
k-NN is a robust classifier, that copes with highly non linear data. It's also interpretable, because we can always show k nearest neighbors, which are an explanation by themselves. It, however, is not flawless. It for instance poorly scales with the size of the training dataset, while in the same time it need, at least in some domains, very big training dataset, as it doesn't generalize any information. 
...
where $d_e$ is euclidean distance, so we simply compute euclidean distance between embedding of images. Here's a scheme of k-NN classifier:
![fig. n: The k-NN Classifier Architecture](./images/3-8-KNNMODEL_1.png)
As we can see on fig. n, new image firstly gets embedded and then a standard classification with k-NN is made. This type of architecture allows us to create robust and interpretable classifier.
...
##### 3.8.5.2.1 K-means
Our problem is supervised one, but we can still use unsupervised approach to get better results. 
...
We use euclidean distance. Prediction for every new observation is simply class of nearest centroid. Algorithm is interpretable, because we can visualize centroids as images. Thanks to using K-means to find subclasses our images are not blurry.
Also because number of all subclasses is much lower than number of records in data set using k-NN only on centroids is much faster. Consider the following dataset:
